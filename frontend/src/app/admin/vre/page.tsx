...
import { VREId } from "@/utils/constants";
...
function page() {
  ...
  async function getData() {
    const params = { searchBarInput, page, limit, selectedRole:VREId };
    setLoading(true);
  ...
  return (
    <div className="w-full px-8 py-2">
      <nav className="h-16 w-full py-1 pt-0 flex justify-between items-center">
        <h3 className="text-[18px] font-[600]">VRE Users</h3>
      </nav>
            ...
            <tbody>
              {!loading && users && users.length !== 0 ? (
                users.map((user, index) => (
                  <tr
                    onClick={()=>router.push(`/admin/vre/performance?userId=${user._id}`)}
                    className="odd:bg-white cursor-pointer odd:dark:bg-gray-900 even:bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700"
                    key={index}
                  >
